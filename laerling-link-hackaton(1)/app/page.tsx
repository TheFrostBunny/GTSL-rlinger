...
const placements = [
  { name: 'GreenTech AS', field: 'IT', desc: 'Søker lærling innen IT og teknologi.', city: 'Oslo', match: 96, initials: 'GT', color: 'bg-[#cbe4dc]' },
  { name: 'Nordic Solutions', field: 'Utvikling', desc: 'Spennende læreplass innen utvikling.', city: 'Bergen', match: 91, initials: 'NS', color: 'bg-[#d8c8ed]' },
  { name: 'Fjord Industri', field: 'Mekanikk', desc: 'Læreplass innen mekanisk fagretning.', city: 'Ålesund', match: 89, initials: 'FJ', color: 'bg-[#dfd4b8]' },
  { name: 'Omsorg Pluss', field: 'Helse', desc: 'Læreplass innen helsearbeidfag ved sykehjem og bofellesskap.', city: 'Trondheim', match: 85, initials: 'OP', color: 'bg-[#cbe4dc]' },
  { name: 'Fjordkraft Elektro', field: 'Elektro', desc: 'Læreplass innen elektrofag med moderne verktøy.', city: 'Trondheim', match: 87, initials: 'FE', color: 'bg-[#d8c8ed]' },
  { name: 'MediaVerket', field: 'Mediaproduksjon', desc: 'Læreplass innen mediaproduksjon og innholdsskaping.', city: 'Bergen', match: 82, initials: 'MV', color: 'bg-[#dfd4b8]' },
]

const candidates = [
  { name: 'Emma Hansen', field: 'IT', desc: 'Motivert kandidat med interesse for programmering.', city: 'Oslo', match: 96, initials: 'EH', color: 'bg-[#cbe4dc]' },
  { name: 'Noah Berg', field: 'Elektro', desc: 'Nysgjerrig kandidat med relevant fagbakgrunn.', city: 'Bergen', match: 92, initials: 'NB', color: 'bg-[#d8c8ed]' },
  { name: 'Sofie Nilsen', field: 'Design og håndverk', desc: 'Kreativ kandidat med sans for detaljer.', city: 'Trondheim', match: 88, initials: 'SN', color: 'bg-[#dfd4b8]' },
]

const applications = [
  { name: 'GreenTech AS', field: 'IT-driftsfaget', city: 'Oslo', date: '12. sep 2026', status: 'Matchet', initials: 'GT', color: 'bg-[#a45bc0]' },
  { name: 'Nordic Solutions', field: 'Utviklerfaget', city: 'Bergen', date: '15. sep 2026', status: 'Venter svar', initials: 'NS', color: 'bg-[#b47bc9]' },
  { name: 'Fjord Industri', field: 'Mekanikerfaget', city: 'Ålesund', date: '18. sep 2026', status: 'Venter svar', initials: 'FJ', color: 'bg-[#512477]' },
  { name: 'Omsorg Pluss', field: 'Helsearbeiderfaget', city: 'Trondheim', date: '20. sep 2026', status: 'Avslått', initials: 'OP', color: 'bg-[#a45bc0]' },
]

const fields = ['Alle', 'IT', 'Bygg og anlegg', 'Helse', 'Mekanikk', 'Mediaproduksjon']

const history = [
  { name: 'GreenTech AS', field: 'IT-driftsfaget', city: 'Oslo', date: '12. sep 2026' },
  { name: 'Nordic Solutions', field: 'Utviklerfaget', city: 'Bergen', date: '15. sep 2026' },
  { name: 'Fjord Industri', field: 'Mekanikerfaget', city: 'Ålesund', date: '18. sep 2026' },
]

function Sidebar({ view, setView, menuOpen, setMenuOpen, page, setPage }: { view: 'learner' | 'company'; setView: (view: 'learner' | 'company') => void; menuOpen: boolean; setMenuOpen: (open: boolean) => void; page: 'home' | 'browse' | 'applications' | 'history'; setPage: (page: 'home' | 'browse' | 'applications' | 'history') => void }) {
...
>('home')
  ...
  const filtered = list.filter(item =>
    ...
    (item.name.toLowerCase().includes(query.toLowerCase()) || item.field.toLowerCase().includes(query.toLowerCase()) || item.city.toLowerCase().includes(query.toLowerCase()))
  )

  if (page === 'applications') {
    const filteredApplications = applications.filter(application =>
      applicationFilter === 'Alle' ||
      (applicationFilter === 'Venter svar' && application.status === 'Venter svar') ||
        ...
        <section className="mx-auto max-w-[1120px] px-6 pb-20 pt-16 lg:ml-[428px] lg:mr-12 lg:px-0">
          ...
          <div className="mt-9 flex flex-col gap-5">
            {filteredApplications.map(application => (
              ...
              >
                <div className={`flex size-20 shrink-0 items-center justify-center rounded-2xl ${application.color} text-2xl font-bold`}>
                ...
                <span className={`rounded-2xl px-5 py-3 font-bold ${
        ...
        <section className="mx-auto max-w-[1300px] px-6 pb-20 pt-16 lg:ml-[428px] lg:mr-12 lg:px-0">
          ...
          <div className="mt-3 flex flex-col gap-4">
            {filtered.map((item) => (
              ...
              >
                <div className={`flex size-11 shrink-0 items-center justify-center rounded-lg ${item.color} text-sm font-bold text-[#18202b]`}>
                  {item.initials}
                </div>
